...
/** Register the small owner-control menu with Telegram; command authorization remains server-side. */
async function registerTelegramCommands(token: string): Promise<void> {
  try {
    ...
    if (!res.ok) log.warn('Telegram command registration failed', { status: res.status });
  } catch (err) {
    log.warn('Telegram command registration failed', { err });
  }
}
...
const CODEX_RATE_LIMIT_COMMAND =
  '(printf \'%s\\n%s\\n\' \'{"id":1,"method":"initialize","params":{"clientInfo":{"name":"nanoclaw-usage","version":"1.0"}}}\' \'{"id":2,"method":"account/rateLimits/read","params":null}\'; sleep 8) | timeout 12 codex app-server --stdio 2>/dev/null';
...
function formatRateLimitWindow(label: string, window: CodexRateLimitWindow | null | undefined): string | null {
  if (!window) return null;
  const remaining = Math.max(0, 100 - window.usedPercent);
  const reset = window.resetsAt ? `; resets ${new Date(window.resetsAt * 1000).toISOString()}` : '';
  return `${label}: ${window.usedPercent}% used, ${remaining}% remaining${reset}`;
}

async function getCodexRateLimitLines(agentGroupId: string): Promise<string[]> {
  const { stdout: containers } = await execFileAsync(
    'docker',
    ['ps', '--filter', `label=nanoclaw-group=${agentGroupId}`, '--format', '{{.ID}}'],
    { timeout: 5000 },
  );
  const containerId = containers.trim().split(/\s+/)[0];
  if (!containerId) throw new Error('no running assistant container');
  const { stdout } = await execFileAsync('docker', ['exec', containerId, 'sh', '-lc', CODEX_RATE_LIMIT_COMMAND], {
    timeout: 15000,
    maxBuffer: 64 * 1024,
  });
    ...
    .map((line) => {
      try {
        return JSON.parse(line) as {
          id?: number;
          result?: { rateLimits?: { primary?: CodexRateLimitWindow; secondary?: CodexRateLimitWindow } };
        };
      } catch {
        return null;
      }
    })
    .find((entry) => entry?.id === 2)?.result?.rateLimits;
  if (!response) throw new Error('Codex did not return rate limits');
  return [
    formatRateLimitWindow('Short window', response.primary),
    formatRateLimitWindow('Long window', response.secondary),
...
async function getControlAgent(platformId: string, instanceKey: string): Promise<string | null> {
  const group = await getMessagingGroupByPlatform('telegram', platformId, instanceKey);
  if (!group) return null;
  const agents = await getMessagingGroupAgents(group.id);
  return agents.length === 1 ? agents[0].agent_group_id : null;
}
...
async function handleControlCommand(
  ...
  const authorized = userId ? (await isOwner(userId)) || (await isGlobalAdmin(userId)) : false;
  if (!authorized) {
    await sendTelegramMessage(token, platformId, { text: 'Only the NanoClaw owner can change assistant settings.' });
    return true;
  }
  const agentGroupId = await getControlAgent(platformId, instanceKey);
  if (!agentGroupId) {
    await sendTelegramMessage(token, platformId, {
      text: 'I could not find exactly one assistant wired to this chat.',
    });
    return true;
  }
  const config = await getContainerConfig(agentGroupId);
  if (!config) {
    await sendTelegramMessage(token, platformId, { text: 'This assistant has no container configuration yet.' });
    return true;
  }
  if (command.name === 'usage') {
    const sessions = await getSessionsByAgentGroup(agentGroupId);
    const active = sessions.filter((session) => session.status === 'active').length;
    let rateLimits = 'Live Codex allowance is temporarily unavailable.';
    try {
      const lines = await getCodexRateLimitLines(agentGroupId);
      rateLimits = lines.length > 0 ? lines.join('\n') : 'Codex did not report a usage window for this account.';
    } catch (err) {
      log.warn('Telegram usage command could not read Codex rate limits', { err, agentGroupId });
    }
    await sendTelegramMessage(token, platformId, {
  ...
  if (command.name === 'model') {
    const model = command.value.toLowerCase() === 'default' ? null : command.value;
    if (model !== null && !/^[a-z0-9][a-z0-9._-]{0,79}$/i.test(model)) {
      await sendTelegramMessage(token, platformId, {
        text: 'Model names may contain letters, numbers, dots, underscores, and hyphens.',
      });
      return true;
    }
    await updateContainerConfigScalars(agentGroupId, { model });
    await restartAgentGroupContainers(agentGroupId, 'Telegram model change');
    await sendTelegramMessage(token, platformId, {
      text: `✅ Model updated to ${model ?? 'the Codex default'}. It will apply to your next request.`,
    });
    return true;
  }
  const effort = command.value.toLowerCase();
  if (effort !== 'default' && !VALID_EFFORTS.has(effort)) {
    await sendTelegramMessage(token, platformId, {
      text: 'Use /effort default, none, minimal, low, medium, high, or xhigh.',
    });
    return true;
  }
  await updateContainerConfigScalars(agentGroupId, { effort: effort === 'default' ? null : effort });
  await restartAgentGroupContainers(agentGroupId, 'Telegram reasoning-effort change');
  await sendTelegramMessage(token, platformId, {
    text: `✅ Reasoning effort updated to ${effort === 'default' ? 'the Codex default' : effort}. It will apply to your next request.`,
  });
  return true;
}
...
export function createTelegramInboundInterceptor(
  ...
  return async (platformId, threadId, message) => {
    ...
    if (!isGroupPlatformId(platformId)) {
      const command = parseControlCommand(text, botUsername);
      if (command) {
        try {
          await handleControlCommand(command, token, platformId, authorUserId, instanceKey);
        } catch (err) {
          log.error('Telegram control command failed', { err, platformId, authorUserId, command: command.name });
          await sendTelegramMessage(token, platformId, { text: 'I could not update that setting. Please try again.' });
        }
        return;
      }
    }
...
export function createTelegramBridge(options: TelegramBridgeOptions = {}): ChannelAdapter | null {
  ...
  void registerTelegramCommands(token);
